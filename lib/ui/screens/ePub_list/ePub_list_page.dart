...
import 'package:woodle/ui/screens/ePub_list/bloc/EPub_list_bloc.dart';
import 'package:woodle/ui/widgets/authors_tile.dart';
import 'package:woodle/ui/widgets/epub_tile.dart';
...
class EPubListPage extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    useEffect(() {
      if (Config.locationId != -1 || address != null)
        context.read<EPubListBloc>().add(
            EPubListEvent.fetchData(
  ...
  Widget _buildBloc(int franchiseId, TextEditingController search) {
    return BlocBuilder<EPubListBloc, EPubListState>(
      builder: (context, state) {
        return state.when(
            ...
            failed: (exceptions) => FailedView(
                ...
                onRetry: () => context.read<EPubListBloc>().add(
                    EPubListEvent.fetchData(franchiseId, categoryId))));
      },
    );
  }

  Widget _buildPage(BuildContext context, List<EPubModel> data,TextEditingController search) {
